...
## 2026-06-09 Phase 7 實際比對結果（蘆洲大愛調和）

來源：`3531142830蘆洲大愛調合/` 來源資料夾（直接掃 Excel）vs staging batch `1ba42faa-...`

**產出行數**

| 流程 | 醫生看 members | 說明 |
|------|--------------|------|
| 舊流程（直接掃 Excel） | 604 | `infer_record_type` 對調和格式有兩個解析缺口 |
| 新流程（staging DB） | 3,102 | tiaohe parser 完整解析 |
| 共同 ID | 600 | — |

**L/M/N/O（cols 12-15）差異**

| 欄位 | 差異筆數 | 方向 | 原因 |
|------|----------|------|------|
| L (col12, 114_count_q1) | 43 | old=空→new=有 | 月份 xlsx 無 `日期`/`總額`，舊流程跳過 |
| M (col13, 115_count) | 28 | old=空→new=有 | 同上 |
| N (col14, 114_avg_amount) | **0** | 完全一致 ✓ | — |
| O (col15, 115_avg_amount) | **0** | 完全一致 ✓ | — |

**AW:BJ（cols 49-62）差異**

| 欄位 | 差異筆數 | 原因 |
|------|----------|------|
| col49-57（個案類別～ASCVD） | 203 | 指定名單用 `身分證號`/`生日` 非 `ID`/`BIRTHDAY`，舊直掃跳過 |
| col58-61（三高～高血糖） | **0，完全一致 ✓** | — |
| col62（與前年+高血壓） | 0 | — |

**結論**：

- 所有差異均來自舊流程「直接掃 Excel 模式」對調和格式的解析缺口，非新流程錯誤。
- N/O 費用平均與 三高/高血壓/高血脂/高血糖（8 個核心欄位）完全一致，驗證新流程費用與疾病計算邏輯正確。
- 新流程透過 tiaohe parser + staging 取得更完整資料，實際上比舊流程更準確。
- 舊流程「正式流程」（先上傳 DB 再讀回）會有不同結果；此次比對特意用直接掃 Excel 以揭露解析缺口。
