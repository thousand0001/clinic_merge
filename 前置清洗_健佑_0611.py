...
- 預防保健名單.CSV：
  - A 結束日作為各類預防保健的檢查日期。
  - B 姓名、C 生日、D 身分證建立耀聖姓名+生日對照。
  - E 預防保健依指定 IC 代碼分流至成健/BC肝/子抹/糞篩/老流。
  - H 主次代碼彙整為主次診斷代碼，不參與預防保健分類。
...
def _merge_prevention_identity_map(
    ...
    if h is None:
        raise ValueError("預防保健名單找不到 A/B/C/D/E/H 必要欄位")
    header = rows[h]
...
def _extend_identity_map_with_internal_ids(
    ...
    for name, bday in sorted(keys):
        ...
        salt = 0
        while True:
            pid = _internal_id(f"{CLINIC_CODE}|{name}|{bday}|{salt}")
...
def _convert_prevention_csv(source_dir: Path, out_dir: Path) -> Dict[str, int]:
    ...
    wb.save(out_dir / "健佑_預防保健_補正.xlsx")
    wb.close()
    ...
    wb_dx.save(out_dir / "健佑_主次診斷_補正.xlsx")
    wb_dx.close()

    # 共用核心需要 HealthCase 工作表；健佑目前未提供檢驗值資料。
    wb_health = Workbook()
    ...
    wb_health.save(out_dir / "健佑_HealthCase_空白.xlsx")
    wb_health.close()
...
def _convert_count_csvs(
    ...
    # 暫存副本中的原始 CSV 已由補正 xlsx 取代，避免共用核心重複計算。
...
def _load_roster_names(path: Path) -> set[str]:
    ...
    try:
        ...
        return {
            ...
            if row and _normalize_name(row[0])
        }
    finally:
        wb.close()


def _apply_selection_overlap_rules(output_path: Path, source_dir: Path) -> int:
    self_names = _load_roster_names(source_dir / "115自選會員.xlsx")
    excluded_names = _load_roster_names(source_dir / "115X不要會員.xlsx")
    overlaps = self_names & excluded_names
    if not overlaps:
        return 0
    ...
    try:
        ...
        for col in range(1, ws.max_column + 1):
            ...
            if header in ("姓名", "會員", "是否為自選會員", "是否為115X"):
                columns[header] = col
        missing = {"姓名", "會員", "是否為自選會員", "是否為115X"} - set(columns)
        if missing:
            raise ValueError("會員總表缺少重疊處理欄位：" + "、".join(sorted(missing)))

        found: set[str] = set()
        for row in range(3, ws.max_row + 1):
            name = _normalize_name(ws.cell(row, columns["姓名"]).value)
            if name not in overlaps:
                continue
            ws.cell(row, columns["會員"]).value = "E1/E2"
            ws.cell(row, columns["是否為自選會員"]).value = "✔"
            ws.cell(row, columns["是否為115X"]).value = "✔"
            found.add(name)
        if found != overlaps:
            raise ValueError(
                "選／不選重疊會員未完整出現在會員總表："
                + "、".join(sorted(overlaps - found))
            )

        self_sheet_name = "自選名單(從會員指標內容Key過來)"
        ws_self = wb[self_sheet_name]
        removed: set[str] = set()
        for row in range(ws_self.max_row, 2, -1):
            name = _normalize_name(ws_self.cell(row, 1).value)
            if name in overlaps:
...
def _post_fill_main_dx(output_path: Path, source_dir: Path) -> int:
    ...
    try:
        ...
        if id_col_out is None or dx_col_out is None:
            raise ValueError("會員總表找不到身分證或病123欄位")
        for row in range(3, ws.max_row + 1):
...
def process_excel(source_path: str, template_path: Optional[str] = None) -> str:
    ...
    if CLINIC_CODE not in source_dir.name or "健佑" not in source_dir.name:
        raise ValueError(f"本程式僅供 {CLINIC_NAME}（{CLINIC_CODE}）使用。")
    ...
    try:
        ...
        if dx_count:
            print(f"  主次診斷原始 H 欄回填：{dx_count} 人", flush=True)
        overlap_count = _apply_selection_overlap_rules(temp_output, source_dir)
        if overlap_count:
            print(f"  選／不選重疊：{overlap_count} 人已標為 E1/E2 並移出自選名單", flush=True)
...
def main() -> None:
    ...
    src = filedialog.askdirectory(title=f"選擇{CLINIC_NAME}耀聖來源資料夾")
    if not src:
        return
