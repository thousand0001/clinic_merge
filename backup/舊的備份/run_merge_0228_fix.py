...
- 計算：AY8/AZ8/BA8（HbA1c 百分位）、BC8/BD8/BE8（LDL 百分位）
...
# 固定設定
# =========================
TEMPLATE_NAME = "選會員樣板0228.xlsx"   # 模板檔名：放在本 .py 同資料夾
SHEET_TARGET = "會員指標"
...
def build_followup_text_from_au(au_txt: str, today: datetime.date, offset_days: int) -> Optional[str]:
    ...
    def build_followup_item_line(
        item_name: str, last_check_date: Optional[datetime.date], today_date: datetime.date
    ) -> str:
        base_date = last_check_date if isinstance(last_check_date, datetime.date) else today_date
        due = base_date + datetime.timedelta(days=offset_days)
        return f"{item_name}:{due.strftime('%Y-%m-%d')}需回診"
...
def calc_hba_percentiles_and_write(ws, cols: Dict[str, Optional[int]], data_start: int, last_row: int) -> None:
    ...
    write_ratio_cell(ws, "BA8", n75, denom)
...
def calc_ldl_percentiles_and_write(ws, cols: Dict[str, Optional[int]], data_start: int, last_row: int) -> None:
    ...
    for r in range(data_start, last_row + 1):
        ...
        f_cat = classify_ascvd(f_raw)  # 'a'/'b' 或 None
    ...
    write_ratio_cell(ws, "BC8", n100, denom)
    write_ratio_cell(ws, "BD8", n105, denom)
    write_ratio_cell(ws, "BE8", n110, denom)
...
def process_excel(source_xlsx_path: str, template_xlsx_path: str) -> str:
    ...
    # ---------- KPI 百分位 ----------
    # 0228 模板的「追蹤提醒」區塊已占用 AY8 等位置；且模板內本來就帶有百分位公式。
    # 為避免覆寫表頭/公式，這裡改成：若 AY8 看起來是百分位區（空白或數值），才寫入；否則跳過。
    try:
        ay8 = ws["AY8"].value
        is_safe = (ay8 is None) or isinstance(ay8, (int, float))
    except Exception:
        is_safe = False
    if is_safe:
