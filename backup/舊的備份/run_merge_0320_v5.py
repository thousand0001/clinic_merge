...
- 套用 0320 樣板，寫入「會員指標」分頁，同步輸出：
  - 百分位名單（LDL / HbA1c 百分位）
  - 醫生看
  - 自選名單
- 填入欄位：疾病樣態 / 主次診斷 / 分數 / 備註 / 追蹤提醒 / AT~AZ 旗標
- KPI：HbA1c（BG/BH）、LDL（BJ/BK）百分位

注意
- Python 3.9 相容；不拆模組，單一檔案維護
- 模板：選會員樣板0320.xlsx（COLUMN_SHIFT=8，AC 之後整體右移 8 欄）
"""
...
# ============================================================
# 欄位位移工具（支援模板在 AC 前插入新欄）
# ============================================================
_SHIFT_START_COL = column_index_from_string("AC")
...
def tpl_col(base_col: str, shift: Optional[int] = None) -> str:
    if shift is None:
        shift = Rules.COLUMN_SHIFT
    idx = col_to_num(base_col)
    if idx >= _SHIFT_START_COL:
        idx += shift
    return num_to_col(idx)


def tpl_cell(base_col: str, row: int, shift: Optional[int] = None) -> str:
    return f"{tpl_col(base_col, shift)}{row}"


def detect_post_ac_shift(followup_cols: Dict[str, Optional[int]]) -> int:
    base_map = {
        "au": col_to_num("AU"),
        "av": col_to_num("AV"),
        "aw": col_to_num("AW"),
    ...
    deltas = []
    for key, base_idx in base_map.items():
        actual = followup_cols.get(key)
        if actual:
            deltas.append(actual - base_idx)
    return max(set(deltas), key=deltas.count) if deltas else 0


# SUMMARY_CELLS 使用「原始欄位名」，tpl_col() 在執行時依 COLUMN_SHIFT 動態換算
# 0313 基準：AY=HbA1c主, AZ=HbA1c目標, BB=LDL主, BC=LDL目標
# 0320 COLUMN_SHIFT=8 後：AY→BG, AZ→BH, BB→BJ, BC→BK, BD→BL
SUMMARY_CELLS = {
    "hba_ratio_main":    ("AY", 8),
    "hba_fraction_main": ("AY", 9),
    "hba_cutoff_label":  ("AZ", 7),
    "hba_ratio_target":  ("AZ", 8),
    "hba_fraction_target": ("AZ", 9),
    "ldl_ratio_main":    ("BB", 8),
    "ldl_fraction_main": ("BB", 9),
    "ldl_cutoff_label":  ("BC", 7),
    "ldl_ratio_target":  ("BC", 8),
    "ldl_fraction_target": ("BC", 9),
    "breakdown_backup":  ("BD", 1),
}


def sc(name: str) -> str:
    col, row = SUMMARY_CELLS[name]
    return tpl_cell(col, row)
...
class Rules:
    ...
    # 模板設定（0320）
    TEMPLATE_NAME: str = "選會員樣板0320.xlsx"
    SHEET_TARGET: str = "會員指標"
    DATA_START_ROW: int = 10

    # 月份申請統計輸出欄位（0320）
    # M=11401-11404件數, N=11401-11412全年件數, O=11501-11504件數
    # S=114金額, T=115金額
    COL_114_COUNT: str = "M"
    COL_114_COUNT_FULL: str = "N"   # 新：全年度114件數
    COL_115_COUNT: str = "O"
    COL_114_AMOUNT: str = "S"
    COL_115_AMOUNT: str = "T"

    # 0320 樣板：在 AC 前新增欄；此值由模板偵測後自動決定
    COLUMN_SHIFT: int = 0
...
def fill_monthly_claim_summary_columns(
    ...
    # 0320 欄位：M=114(Q1件數), N=114(全年件數), O=115(Q1件數), S=114金額, T=115金額
    col_m    = cols.get("m_count_114")       # M：11401-11404 件數
    col_n_fy = cols.get("m_count_114_full")  # N：11401-11412 全年件數（新）
    col_o    = cols.get("n_count_115")       # O：11501-11504 件數
    col_s    = cols.get("r_amount_114")      # S：114 金額
    col_t    = cols.get("s_amount_115")      # T：115 金額

    if not all([col_m, col_o, col_s, col_t, cols.get("id")]):
        raise ValueError("模板找不到 M/O/S/T 或 ID 欄位，無法填入月份申請統計")
    ...
    for rr in range(data_start, last_row + 1):
        ...
        ws.cell(rr, col_s).value = _to_excel_number(v114a) if v114a != 0 else None
        ws.cell(rr, col_t).value = _to_excel_number(v115a) if v115a != 0 else None
...
def _detect_basic_cols(ws, header_row: int) -> Dict[str, Optional[int]]:
    ...
    return {
        ...
        "last_visit": kw(ws, header_row, ["最後就診日"]),
    }


def _detect_screening_cols(ws, header_row: int) -> Dict[str, Optional[int]]:
    """
    5大篩檢欄：先嘗試關鍵字偵測，若找不到則 fallback 到多列掃描再到固定欄位。
    0320 樣板固定位置：U=成人, V=子宮, W=老人流感, X=糞便, Y=肝炎
    """
    kw     = find_col_by_keywords
    kw_any = find_col_by_keywords_any_row
    max_scan = header_row + 3  # 多掃幾列以防標題跨列

    def _find_screening(keywords: List[str], fallback_col: str) -> Optional[int]:
        return (
            kw(ws, header_row, keywords)
            or kw_any(ws, max_scan, keywords)
            or column_index_from_string(fallback_col)
        )

    return {
        "adult": _find_screening(["成人", "保健"], "U"),
        "pap":   _find_screening(["子宮", "抹片"], "V"),
        "flu":   _find_screening(["老人", "流感"], "W"),
        "fit":   _find_screening(["糞便", "潛血"], "X"),
        "hep":   _find_screening(["BC肝炎"],       "Y"),
    }
...
def _detect_output_cols(ws, header_row: int, max_scan_row: int) -> Dict[str, Optional[int]]:
    ...
    return {
        ...
        "breakdown": (kw(ws, header_row, ["分數說明"])
                        ...
                        or column_index_from_string(tpl_col("BD"))),  # 動態換算
        ...
        "main_sub_dx": (kw(ws, header_row, ["主次診斷"])
...
def detect_template_columns(ws, data_start: int) -> Dict[str, Optional[int]]:
    ...
    Rules.COLUMN_SHIFT = detect_post_ac_shift(followup_cols)
    print(f"偵測模板欄位位移：AC 之後整體右移 {Rules.COLUMN_SHIFT} 欄")
    ...
    cols["m_count_114_full"] = column_index_from_string(Rules.COL_114_COUNT_FULL)
    ...
    # KPI 標記欄：透過 tpl_col() 動態計算（COLUMN_SHIFT 已在上方偵測）
    cols["ay_mark"] = column_index_from_string(tpl_col("AY"))
    cols["az_mark"] = column_index_from_string(tpl_col("AZ"))
    cols["bb_mark"] = column_index_from_string(tpl_col("BB"))
    cols["bc_mark"] = column_index_from_string(tpl_col("BC"))
...
def calc_hba_kpi_ay_az(
    ...
    _write_ratio(ws, sc("hba_ratio_main"), numer_ay, denom)
    ws[sc("hba_fraction_main")].value = f"{numer_ay}/{denom}" if denom > 0 else "0/0"

    if denom <= 0:
        ws[sc("hba_cutoff_label")].value = None
        _write_ratio(ws, sc("hba_ratio_target"), 0, 0)
        ws[sc("hba_fraction_target")].value = "0/0"
    ...
    ws[sc("hba_cutoff_label")].value = f"<={cutoff:.2f}"
    _write_ratio(ws, sc("hba_ratio_target"), k, denom)
    ws[sc("hba_fraction_target")].value = f"{k}/{denom}"
...
def calc_ldl_percentiles(
    ...
    _write_ratio(ws, sc("ldl_ratio_main"), numer_bb, denom)
    ws[sc("ldl_fraction_main")].value = f"{numer_bb}/{denom}" if denom > 0 else "0/0"

    if denom <= 0:
        ws[sc("ldl_cutoff_label")].value = None
        _write_ratio(ws, sc("ldl_ratio_target"), 0, 0)
        ws[sc("ldl_fraction_target")].value = "0/0"
    ...
    ws[sc("ldl_cutoff_label")].value = f"<={cutoff:.1f}".replace(".0", "")
    _write_ratio(ws, sc("ldl_ratio_target"), k, denom)
    ws[sc("ldl_fraction_target")].value = f"{k}/{denom}"
...
# ============================================================
# 欄位字母 → cols key 共用對照表（醫生看 / 自選名單共用）
# ============================================================
_LETTER_TO_COL_KEY: Dict[str, str] = {
    "B":  "name",
    "D":  "dmk_code",
    "E":  "ascvd",
    "G":  "bday",
...
def _build_src_col_map(
    ...
    """
    根據 col_map（來源欄字母, 目標欄字母）和 cols dict，
    建立來源欄字母 → 實際欄號的對照表。
    優先用 cols dict（已含 COLUMN_SHIFT），找不到則直接用欄字母轉欄號。
    """
    result: Dict[str, Optional[int]] = {}
    for src_letter, _ in col_map:
        key = _LETTER_TO_COL_KEY.get(src_letter)
        if key and cols.get(key):
            result[src_letter] = cols[key]
        else:
            result[src_letter] = column_index_from_string(src_letter)
    return result
...
def _copy_sheet_rows(
    ...
    for src_row in range(data_start, last_row + 1):
        ...
        for src_letter, dst_letter in col_map:
            src_c = src_col_map.get(src_letter)
            dst_c = column_index_from_string(dst_letter)
            if src_c:
                ws_out.cell(dst_row, dst_c).value = ws_main.cell(src_row, src_c).value
        dst_row += 1
...
def populate_percentile_sheet(
    ...
    _set_percentile_title_rich(ws, "A1", "LDL百分位", ws_main[sc("ldl_fraction_main")].value, ws_main[sc("ldl_fraction_target")].value)
    _set_percentile_title_rich(ws, "N1", "HBA1C百分位", ws_main[sc("hba_fraction_main")].value, ws_main[sc("hba_fraction_target")].value)
...
def collect_all_members(wb_src) -> Dict[str, Dict[str, Any]]:
    ...
    優先順序（後蓋前）：月份分頁 < 其他名單 < 會員名單
    月份分頁同一人以最後月份為主。
    回傳 {id: {name, bday, e_code, abc, tel, dmk_raw}}
    """
    ...
    for sname in month_sheets:
        ...
        if _month_id_col is None:
            ...
            _month_name_col = find_column_exact(hmap, ["姓名"])
        if _month_id_col is None:
            continue
        for r in range(_month_header_row + 1, sh.max_row + 1):
            ...
            if not pid or not is_valid_tw_id(pid):
                continue
            name = normalize_text(sh.cell(r, _month_name_col).value) if _month_name_col else ""
            members[pid] = {"name": name, "bday": None, "e_code": None,
                            "abc": None, "tel": None, "dmk_raw": None}
    ...
    for sname in ["ascvd", "自選名單", "115X", "P4P收案", "P4P追蹤"]:
        ...
        for pid, name in m.items():
            if pid not in members:
                members[pid] = {"name": name, "bday": None, "e_code": None,
                                "abc": None, "tel": None, "dmk_raw": None}
            elif name and not members[pid].get("name"):
                members[pid]["name"] = name

    # 3. 會員名單（最高優先，完整覆蓋）
    if "會員名單" in snames:
        sh = wb_src["會員名單"]
        header_row = _find_header_row_contains_any(sh, [id_aliases], search_rows=10)
        if header_row is None:
            header_row = 5
        hmap = build_header_map(sh, header_row)
        id_col_cand = find_column_exact(hmap, id_aliases)
        ...
        tel_col    = find_column_exact(hmap, ["電話"])
        ...
        if id_col:
            for r in range(header_row + 1, sh.max_row + 1):
                ...
                if not pid or not is_valid_tw_id(pid):
                    continue
                members[pid] = {
                    "name":    normalize_text(sh.cell(r, name_col).value) if name_col else "",
                    "bday":    parse_date(sh.cell(r, bday_col).value) if bday_col else None,
                    "e_code":  parse_disease_code(sh.cell(r, dmk_col).value) if dmk_col else None,
                    "abc":     sh.cell(r, abc_col).value if abc_col else None,
                    "tel":     normalize_text(sh.cell(r, tel_col).value) if tel_col else None,
                    "dmk_raw": sh.cell(r, dmk_col).value if dmk_col else None,
                    "cnt":     sh.cell(r, cnt_col).value if cnt_col else None,
                    "clinic":  normalize_text(sh.cell(1, 1).value),
                }
...
# 電話整合（行動電話優先）
# ============================================================
def build_phone_map(sh_phone: Any) -> Dict[str, str]:
    """從行動電話 sheet 建立 {id: tel} 對應"""
    if sh_phone is None:
        return {}
    ...
    tel_col = find_column_exact(hmap, ["電話", "手機", "行動電話"])
    if id_col is None or tel_col is None:
        return {}
    result: Dict[str, str] = {}
    for r in range(header_row + 1, sh_phone.max_row + 1):
        pid = normalize_id(sh_phone.cell(r, id_col).value)
        tel = normalize_text(sh_phone.cell(r, tel_col).value)
        if pid and is_valid_tw_id(pid) and tel:
            result[pid] = tel
    return result
...
def _fill_extra_flags(
    ...
    for pid, rows in id_to_rows.items():
        ...
        for rr in rows:
            ...
            safe_set(ws, rr, col_ax, is_114)
            safe_set(ws, rr, col_ay, is_self)
            safe_set(ws, rr, col_az, is_115x)
...
def _fill_member_basic(
    ...
    電話以 phone_map（行動電話）優先，其次用 all_members 內的 tel。
    """
    id_to_rows: Dict[str, List[int]] = {}
    ...
    for pid, info in all_members.items():
        ...
        abc    = info.get("abc")
        tel    = phone_map.get(pid) or info.get("tel") or ""  # 行動電話優先
        cnt    = info.get("cnt")
        ...
        safe_set(ws, out_r, cols.get("clinic"),   clinic)
        safe_set(ws, out_r, cols.get("name"),     name or None)
        safe_set(ws, out_r, cols.get("id"),       pid)
        safe_set(ws, out_r, cols.get("bday"),     bday)
        safe_set(ws, out_r, cols.get("age"),      age if age >= 0 else None)
        safe_set(ws, out_r, cols.get("tel"),      tel or None)
        safe_set(ws, out_r, cols.get("abc"),      abc)
        safe_set(ws, out_r, cols.get("dmk_code"), e_code.value if e_code else None)
        safe_set(ws, out_r, cols.get("cnt"),      cnt)
...
def load_source(source_path: str) -> Dict[str, Any]:
    ...
    return {
        "wb_src": wb_src,
        "sh_member":     wb_src["會員名單"]  if "會員名單"  in snames else None,
        "sh_ascvd":      wb_src["ascvd"]     if "ascvd"     in snames else None,
        "sh_health":     wb_src["HealthCase"],
        "sh_main_sub_dx":wb_src["主次診斷"]  if "主次診斷"  in snames else None,
        "sh_phone":      wb_src["行動電話"]  if "行動電話"  in snames else None,
        "sh_self_select":wb_src["自選名單"]  if "自選名單"  in snames else None,
        "sh_115x":       wb_src["115X"]      if "115X"      in snames else None,
        "sh_p4p_enroll": wb_src["P4P收案"]   if "P4P收案"   in snames else None,
        "sh_p4p_track":  wb_src["P4P追蹤"]   if "P4P追蹤"   in snames else None,
        "screening_sheets": {
            "adult": wb_src["成人健檢"],
            "pap":   wb_src["子宮抹片"],
            "flu":   wb_src["老人流感"],
...
def fill_basic_data(source_ctx: Dict[str, Any], template_ctx: Dict[str, Any], now: datetime.date) -> Dict[str, Any]:
    phone_map  = build_phone_map(source_ctx.get("sh_phone"))
    clinic_val = ""
    if source_ctx.get("sh_member") is not None:
...
def fill_external_data(source_ctx: Dict[str, Any], template_ctx: Dict[str, Any], runtime_ctx: Dict[str, Any]) -> None:
    ...
    if source_ctx.get("sh_ascvd"):
        _fill_ascvd(ws, source_ctx["sh_ascvd"], cols, id_to_rows, meta)
    _fill_main_sub_dx(ws, source_ctx.get("sh_main_sub_dx"), cols, id_to_rows)
    ...
    ascvd_ids       = build_id_set(source_ctx.get("sh_ascvd"),      id_aliases)
    self_select_ids = build_id_set(source_ctx.get("sh_self_select"), id_aliases)
    x115_ids        = build_id_set(source_ctx.get("sh_115x"),        id_aliases)
    p4p_map         = build_p4p_map(source_ctx.get("sh_p4p_enroll"),
                                    source_ctx.get("sh_p4p_track"))
    _fill_extra_flags(ws, cols, data_start, last_row, id_to_rows,
...
def compute_kpis(template_ctx: Dict[str, Any], runtime_ctx: Dict[str, Any], source_ctx: Optional[Dict[str, Any]] = None) -> None:
    ws = template_ctx["ws"]
    cols = template_ctx["cols"]
    data_start = template_ctx["data_start"]
    last_row = runtime_ctx["last_row"]
    ...
    if source_ctx:
        populate_self_select_sheet(template_ctx["wb_tpl"], ws, cols, data_start, last_row, source_ctx.get("sh_self_select"))
