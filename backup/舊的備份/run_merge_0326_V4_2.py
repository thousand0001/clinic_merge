...
class Rules:
    ...
    # 模板設定（0325）
    TEMPLATE_NAME: str = "選會員樣板0325.xlsx"
...
def _set_percentile_title_rich(ws, cell_ref: str, prefix: str, red_num: Any, blue_num: Any) -> None:
    ws[cell_ref].value = f"{prefix}({_ratio_numerator_text(red_num)}人 /{_ratio_numerator_text(blue_num)}人)"
...
def finalize_and_save(source_path: str, template_ctx: TemplateContext, now_dt: datetime.datetime) -> str:
    ...
    out_path = os.path.join(base_dir, f"選會員{now_dt.strftime('%m%d_%H%M')}.xlsx")
