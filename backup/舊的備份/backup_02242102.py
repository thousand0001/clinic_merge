...
# =========================
# AV / AW：從 AU 文字生成「同樣格式」的追蹤文字
# =========================
DATE_RE = re.compile(r"(\d{4}-\d{2}-\d{2})")

def au_should_skip_followup(au_txt: str) -> bool:
    """
    AU 不須通知 or 超過兩年未檢查 -> AV/AW 照舊（不填）
    """
    if not au_txt:
        return True
    s = au_txt.replace(" ", "")
...
def process_excel(source_xlsx_path: str, template_xlsx_path: str) -> str:
    ...
    for rr in range(DATA_START, last_row + 1):
        ...
        # ✅ AV/AW：同 AU 顯示方式，只是日期延後
        # 規則：
        # 1) AU 不須通知 or 超過兩年未檢查 -> AV/AW 不填
        # 2) AU 行內有 yyyy-mm-dd -> AV=+28, AW=+56（每行各自算）
        # 3) AU 行內「可立刻通知」 -> AV=今天+28, AW=今天+56（仍顯示 YYYY-MM-DD需回診）
    ...
    # =========================
    # AY8 / AZ8 / BA8：HbA1c 百分位（<=7 / <=7.3 / <=7.5）
    # 條件：
    # - 疾病樣態碼：1 或 3
    # - ASCVD：0 / "0" / 空白 / None 視為 0
    # - HbA1c 日期：年份 2025 或 2026
    # - HbA1c 數值：可轉成數字才算分母
    # =========================
    denom = 0
    numer_7 = 0
    ...
    for r in range(DATA_START, last_row + 1):
        ...
        # ASCVD 必須為 0 / 空白
        if not _is_ascvd_zero(ws.cell(r, col_ascvd).value):
            continue

        # HbA1c 日期必須是 2025 或 2026
        hba_dt = parse_date(ws.cell(r, col_hba_dt).value) if col_hba_dt else None
        if not (isinstance(hba_dt, datetime.date) and hba_dt.year in (2025, 2026)):
            continue

        # HbA1c 數值必須是數字
        hba_val = parse_float(ws.cell(r, col_hba).value) if col_hba else None
        if hba_val is None:
            continue
    ...
    # 寫入 AY8 / AZ8 / BA8
    if denom > 0:
        ws["AY8"].value = numer_7 / denom
        ws["AZ8"].value = numer_73 / denom
        ws["BA8"].value = numer_75 / denom
    else:
        ws["AY8"].value = 0
        ws["AZ8"].value = 0
        ws["BA8"].value = 0

    ws["AY8"].number_format = "0.00%"
    ws["AZ8"].number_format = "0.00%"
    ws["BA8"].number_format = "0.00%"



    # =========================
    # BC8 / BD8 / BE8：LDL 百分位（<=100 / <=105 / <=110）
    # =========================
    denom_ldl = 0
    numer_100 = 0
    numer_105 = 0
    ...
    for r in range(DATA_START, last_row + 1):
        ...
        # ASCVD 分類（0 / a / b）
        f_val = ws.cell(r, col_ascvd).value
        f_cat = classify_ascvd(f_val)  # a/b 或 None
        ascvd_is_zero = (f_cat is None) and (str(f_val).strip() in ("", "0", "0.0") or f_val is None)
        ...
        # LDL 日期：2025 或 2026
        ldl_dt = parse_date(ws.cell(r, col_ldl_dt).value) if col_ldl_dt else None
        if not (isinstance(ldl_dt, datetime.date) and ldl_dt.year in (2025, 2026)):
            continue

        # LDL 數值必須是數字
        ldl_val = parse_float(ws.cell(r, col_ldl).value) if col_ldl else None
        if ldl_val is None:
            continue
    ...
    if denom_ldl > 0:
        ws["BC8"].value = numer_100 / denom_ldl
        ws["BD8"].value = numer_105 / denom_ldl
        ws["BE8"].value = numer_110 / denom_ldl
    else:
        ws["BC8"].value = 0
        ws["BD8"].value = 0
        ws["BE8"].value = 0

    ws["BC8"].number_format = "0.00%"
    ws["BD8"].number_format = "0.00%"
    ws["BE8"].number_format = "0.00%"
