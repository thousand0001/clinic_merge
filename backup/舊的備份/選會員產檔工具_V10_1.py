...
def _find_header_row_contains_any(sheet, alias_groups: List[List[str]], search_rows: int = 30) -> Optional[int]:
    for r in range(1, min(search_rows, sheet.max_row) + 1):
        ...
        for group in alias_groups:
            if not any(alias.replace(" ", "").replace("\n", "") in row_headers for alias in group):
                ok = False
                break
        if ok:
            return r
...
def _compute_all_derived(
    ...
    for rr in range(data_start, last_row + 1):
        ...
        safe_set(ws, rr, cols.get("ay_mark"), "✔" if rr in kpi_marks.get("ay", set()) else None)
        safe_set(ws, rr, cols.get("az_mark"), "✔" if rr in kpi_marks.get("az", set()) else None)
        safe_set(ws, rr, cols.get("bb_mark"), "✔" if rr in kpi_marks.get("bb", set()) else None)
        safe_set(ws, rr, cols.get("bc_mark"), "✔" if rr in kpi_marks.get("bc", set()) else None)
...
def process_excel(source_path: str, template_path: str) -> str:
    ...
    # 主表保留模板原始線條，不再重畫 border；改為開啟 Excel 原生格線
    for sht in wb_tpl.worksheets:
        sht.sheet_view.showGridLines = True
