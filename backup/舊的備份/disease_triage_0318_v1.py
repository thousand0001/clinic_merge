...
def remark_code(row: pd.Series, sheet_name: str, base_year: int,
                _ref: Optional[pd.Timestamp] = None) -> int:
...
def build_master_df(path_a: Path, path_b: Path) -> pd.DataFrame:
    ...
    # 在終端機提示找不到疾病樣態的筆數，不額外產生 issues 檔
...
def resolve_screening_sheet_name(
    path_a: Path,
    target_name: str,
    norm_to_real: Optional[Dict[str, str]] = None,
) -> Optional[str]:
    """norm_to_real 可由外部傳入以避免重複 open_workbook。"""
...
def build_all_screening_lookup(path_a: Path) -> pd.DataFrame:
    """
    建立 5 項篩檢的共用 lookup，回傳欄位：ID + SCREENING_EXPORT_COLS
    - 先收集所有 ID，再逐一 left merge，確保不產生多餘列
    """
    lookup_specs = [
        ("成人健檢", "成人預防保健(6分)"),
        ("子宮抹片", "子宮頸抹片檢查率(6分)"),
    ...
    # 先收集所有 lookup，取所有 ID 的聯集建主表
    # 只開一次 workbook，取得 sheet 名稱對應表
    _xl = open_workbook(path_a)
    _norm_to_real = {_norm_colname(sh): sh for sh in _xl.sheet_names}

    lookups: Dict[str, pd.DataFrame] = {}
    all_ids: set = set()
    for logical_sheet_name, out_col in lookup_specs:
        lkp = build_screening_lookup(path_a, logical_sheet_name, _norm_to_real)
        if lkp.empty:
    ...
    # 以所有出現過的 ID 建主表，逐一 left merge
    result = pd.DataFrame({"ID": sorted(all_ids)})
    for out_col in SCREENING_EXPORT_COLS:
        if out_col in lookups:
            result = result.merge(lookups[out_col], on="ID", how="left")
        else:
...
def split_into_sheets(df: pd.DataFrame, base_year: int) -> Tuple[Dict[str, pd.DataFrame], Dict[str, Dict[str, int]]]:
    ...
    _ref_ts = pd.Timestamp(f"{base_year}-06-30")  # 預建一次，apply 閉包共用

    for name, d in sheets.items():
        if len(d) > 0:
            x = len(required_items(name))
            codes = d.apply(lambda r: remark_code(r, name, base_year, _ref_ts), axis=1)
...
def style_triage_sheet(ws, sheet_name: str):
    ...
    items = required_items(sheet_name)
    _style_ref = pd.Timestamp(f"{BASE_YEAR}-06-30")  # 預建一次
    ...
    for r in range(2, last_data_row + 1):
        ...
        age = age_on(bday_dt, _style_ref)
        h_target = 8.0 if (age is not None and age >= 80) else 7.0
        # 直接計算 LDL 目標，避免每列建 pd.Series
...
def write_output(out_path: Path, sheets: Dict[str, pd.DataFrame], analysis_counts: Dict[str, Dict[str, int]], no_disease_df: Optional[pd.DataFrame] = None):
    ...
    _fix_set = set(_all_fix)  # 用 set 加速 `in` 檢查
    ...
    for name in SHEET_ORDER:
        ...
        # 1) '0'/'0.0' -> '-'
        fix_cols = [c for c in df.columns if c in _fix_set]
