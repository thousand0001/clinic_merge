...
def build_no_disease_df(df: pd.DataFrame, path_a: Path) -> Tuple[pd.DataFrame, List[str]]:
    """
    回傳 (DataFrame, skipped_cols)
    - 若 A 檔缺少某篩檢 sheet，欄位仍保留於輸出，整欄填 '-'
    - skipped_cols：找不到 sheet 的欄名清單（供呼叫端顯示提示用）
    """
    base = df[
        (~df["疾病樣態"].isin([1, 2, 3])) &
        (~df["ASCVD"].isin(["a", "b"]))
    ][["ID", "疾病樣態", "ASCVD", "姓名", "生日"]].copy()
    ...
    _NA_STRS = {"NaT", "nan", "None", "<NA>", "-"}

    for logical_sheet_name, out_col in lookup_specs:
        resolved = resolve_screening_sheet_name(path_a, logical_sheet_name)
        if not resolved:
            # ✅ Sheet 找不到：欄位保留，整欄填 '-'
            base[out_col] = "-"
            skipped_cols.append(out_col)
            continue

        lookup = build_screening_lookup(path_a, logical_sheet_name).rename(columns={"篩檢日期": out_col})
        base = base.merge(lookup, on="ID", how="left")

        if out_col in base.columns:
            s = base[out_col].astype(str).str.strip()
            base.loc[s.isin(_NA_STRS), out_col] = pd.NA

    result = (base[NO_DISEASE_EXPORT_COLS]
              .sort_values("ID", ascending=True, kind="mergesort")
              .reset_index(drop=True))
    return result, skipped_cols
...
def split_into_sheets(df: pd.DataFrame, base_year: int) -> Tuple[Dict[str, pd.DataFrame], Dict[str, Dict[str, int]]]:
    ...
    for name, d in sheets.items():
        ...
        sheets[name] = d[EXPORT_COLS[:12]].copy()  # 先只取原本12欄，篩檢欄後面再 merge

    return sheets, analysis_counts


def attach_screening_cols(
    sheets: Dict[str, pd.DataFrame],
    path_a: Path,
) -> Tuple[Dict[str, pd.DataFrame], List[str]]:
    """
    將 5 個篩檢欄 merge 進所有分流 sheet。
    - sheet 找不到：整欄填 '-'
    - 回傳 (sheets_with_screening, skipped_cols)
    """
    lookup_specs = [
        ("成人健檢",  "成人預防保健(6分)"),
        ("子宮抹片",  "子宮頸抹片檢查率(6分)"),
    ...
    # 預先建好所有 lookup（每個篩檢 sheet 只讀一次）
    lookups: Dict[str, pd.DataFrame] = {}
    for logical_name, out_col in lookup_specs:
        resolved = resolve_screening_sheet_name(path_a, logical_name)
        if not resolved:
            skipped_cols.append(out_col)
            lookups[out_col] = None  # 標記為缺失
        else:
            lkp = build_screening_lookup(path_a, logical_name).rename(columns={"篩檢日期": out_col})
            lookups[out_col] = lkp

    for name, df in sheets.items():
        for _, out_col in lookup_specs:
            lkp = lookups[out_col]
            if lkp is None:
                df[out_col] = "-"
            else:
                df = df.merge(lkp, on="ID", how="left")
                if out_col in df.columns:
                    s = df[out_col].astype(str).str.strip()
                    df.loc[s.isin(_NA_STRS), out_col] = pd.NA
        sheets[name] = df[EXPORT_COLS].copy()

    return sheets, skipped_cols
...
def style_triage_sheet(ws, sheet_name: str):
    ...
    for r in range(2, last_data_row + 1):
        ...
        # ✅ 篩檢欄：今年日期→綠；其餘不填色
        for sc in SCREENING_COLS:
            sc_idx = col_idx(sc)
            if sc_idx:
                cell = ws.cell(r, sc_idx)
                if _parse_year(cell.value) == BASE_YEAR:
                    cell.fill = FILL_GREEN
...
def style_no_disease_sheet(ws):
    ...
    fill_2026 = PatternFill("solid", fgColor="C6E0B4")
...
def write_output(out_path: Path, sheets: Dict[str, pd.DataFrame], analysis_counts: Dict[str, Dict[str, int]], no_disease_df: Optional[pd.DataFrame] = None):
    ...
    for name in SHEET_ORDER:
        ...
        date_cols = ["最近一次HbA1c檢查日期", "最近一次LDL檢查日期", "最近一次UACR檢查日期"]
        val_cols = ["最近一次HbA1c檢查結果(%)", "最近一次LDL檢查結果(mg/dL)", "最近一次UACR檢查結果(mg/gm)"]
        fix_cols = [c for c in (text_cols + date_cols + val_cols + SCREENING_COLS) if c in df.columns]
        ...
        for sc in SCREENING_COLS:
            if sc in headers:
                sc_col = get_column_letter(headers.index(sc) + 1)
                ws.column_dimensions[sc_col].width = 16
...
def main():
    ...
    try:
        ...
        print("▶ 合併篩檢欄位至各分流頁...")
        sheets, skipped_screening = attach_screening_cols(sheets, Path(path_a))
        if skipped_screening:
            for col in skipped_screening:
                print(f"  ⚠ 篩檢 Sheet 找不到，欄位填 '-'：{col}")
        else:
            print("  ✓ 篩檢欄全數合併完成")

        print("▶ 建立無疾病分頁...")
        no_disease_df, skipped_cols = build_no_disease_df(df, Path(path_a))
        print(f"  ✓ 無疾病共 {len(no_disease_df)} 筆")
        if skipped_cols:
            for col in skipped_cols:
                print(f"  ⚠ Sheet 找不到，欄位填 '-'：{col}")

        print("▶ 寫出 Excel...")
        write_output(out_path, sheets, analysis_counts, no_disease_df)
        print(f"  ✓ 輸出完成：{out_path}")

        all_skipped = list(dict.fromkeys(skipped_screening + skipped_cols))  # 去重保序
        msg = f"報表完成（年度固定 {BASE_YEAR}）：\n{out_path}"
        if all_skipped:
            msg += "\n\n⚠️ 以下篩檢 sheet 找不到，對應欄保留但填 '-'：\n"
            msg += "\n".join(f"  · {c}" for c in all_skipped)

        if EXPORT_ISSUES and issues_df is not None and not issues_df.empty:
            issues_path = out_dir / f"未達控制{ts}_issues.csv"
            issues_df.to_csv(issues_path, index=False, encoding="utf-8-sig")
            msg += f"\n\n另產生 issues：\n{issues_path}"
            print(f"  ⚠ issues 輸出：{issues_path}")
