# -*- coding: utf-8 -*-
"""
disease_triage_gui_v4_stable_ods_color.py

✅ 可讀 .xlsx / .xls / .os（.ods 需 pip install odfpy）
✅ 7 分流 + 分析表（邏輯不動）
✅ 分流頁樣式：備註左對齊可換行，其餘置中；不加粗/不放大姓名
✅ 避免 used range：只處理實際資料列
✅ 輸出：未達控制mmdd_hhmm.xlsx（放 B 檔同資料夾），並自動開啟
✅ 分析表：刪除原本第2列；原本第4,8,12,16,20,24,28列(備註文字列)列高=1.2倍
✅ 寫出前：日期/文字欄位值為 '0' 或 '0.0' 轉成 '-'
✅ 備註欄寬固定 185pt（openpyxl 欄寬近似：26）
...
def build_master_df(path_a: Path, path_b: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not miss_b.empty:
        miss_b["問題"] = "B檔找不到此ID的疾病樣態/ASCVD（merge不到）"
...
def style_triage_sheet(ws, sheet_name: str):
    ...
    # helper：日期上色（今年綠/去年粉/其他黃）
    def paint_date(row_r: int, col_dt: Optional[int]):
        if not col_dt:
            return
        y = _parse_year(ws.cell(row_r, col_dt).value)
        if y == BASE_YEAR:
            ws.cell(row_r, col_dt).fill = FILL_GREEN
        elif y == BASE_YEAR - 1:
            ws.cell(row_r, col_dt).fill = FILL_PINK
        else:
            ws.cell(row_r, col_dt).fill = FILL_YELLOW
    ...
    def paint_value_if_bad(row_r: int, col_val: Optional[int], col_dt: Optional[int], threshold: float):
        ...
        if y == BASE_YEAR and (v is not None) and v >= threshold:
            ws.cell(row_r, col_val).fill = FILL_ORANGE
    ...
    for r in range(2, last_data_row + 1):
        ...
        bday_dt = pd.to_datetime(bday_str, errors="coerce") if bday_str not in (None, "-", "0", "0.0") else pd.NaT
...
def build_analysis_sheet(ws, analysis_counts: Dict[str, Dict[str, int]], base_year: int):
    ...
    # ✅ 原本第2列刪除：從第2列開始畫（不留空白 row 2）
    r = 2
    base_h = 33
    remark_h = base_h * 1.2  # ✅ 原本第4/8/12/...（每組的備註文字列）加高
...
# ------------------------- write output -------------------------
def write_output(out_path: Path, sheets: Dict[str, pd.DataFrame], analysis_counts: Dict[str, Dict[str, int]]):
    wb = Workbook()
    wb.remove(wb.active)
    ...
    for name in SHEET_ORDER:
        ...
        for col in fix_cols:
            # 用 astype(str) 做比對，但不把整欄 permanently 轉成 string dtype
            s = df[col].astype(str).str.strip()
            mask = df[col].notna() & s.isin(["0", "0.0"])
            # 這裡可能會有 dtype 警告（若 col 是純 int），所以先轉成 object 再塞字串
            if mask.any():
                df[col] = df[col].astype("object")
                df.loc[mask, col] = "-"

        # =============================
        # 2) 寫入 Excel（逐格寫入，100% 避免 <NA>）
        # =============================
        # header
        ...
        # 備註欄固定 185pt（openpyxl 欄寬近似：26）
        if "備註" in headers:
            remark_col = get_column_letter(headers.index("備註") + 1)
            ws.column_dimensions[remark_col].width = 30

        # =============================
        # 4) 分頁樣式 + 指定欄位上色（不動 border）
        # =============================
...
def main():
    ...
    try:
        ...
        write_output(out_path, sheets, analysis_counts)
        ...
        messagebox.showerror("錯誤(Traceback)", traceback.format_exc())
