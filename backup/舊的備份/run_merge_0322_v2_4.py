...
def apply_full_grid(ws, max_row: int, max_col: int) -> None:
    ...
    # 用暫存格取得 _style 索引後批次套用，不逐格建立 Border 物件
    _tmp = ws.cell(1, 1)
    _tmp.border = grid
    _grid_style = _tmp._style
    for r in range(1, max_row + 1):
        for c in range(1, max_col + 1):
            cell = ws.cell(r, c)
            if cell.value is not None or r == 1:
                cell._style = _grid_style
...
def populate_percentile_sheet(
    ...
    style_src_row = 5
    # 預快取 row5 每欄的 _style 索引，後續列直接套用，不重複讀取
    _src_height = ws.row_dimensions[style_src_row].height or 66
    _src_styles = [ws.cell(style_src_row, c)._style for c in range(1, 25)]
    for idx in range(data_rows):
        row = 5 + idx
        if row != style_src_row:
            ws.row_dimensions[row].height = _src_height
            for c in range(1, 25):
                ws.cell(row, c)._style = _src_styles[c - 1]
