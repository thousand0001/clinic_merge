# -*- coding: utf-8 -*-
"""
選會員產檔工具（結構化單檔版）— V10.2 refactor（0307樣板）
...
def _compact_keywords(items: List[str]) -> List[str]:
    return [normalize_header(k) for k in items if k]
...
def find_col_by_keywords(ws, header_row: int, keywords: List[str]) -> Optional[int]:
    ...
    keys = _compact_keywords(keywords)
...
def find_header_row_contains(sheet, must_have: List[str], search_rows: int = 250) -> Optional[int]:
    must = _compact_keywords(must_have)
    for r in range(1, min(search_rows, sheet.max_row) + 1):
        found = set()
        for c in range(1, sheet.max_column + 1):
...
def _apply_percentile_date_formats(ws, start_row: int, data_rows: int) -> None:
...
def _enable_gridlines(wb, sheet_names: Tuple[str, ...]) -> None:
    for sht_name in sheet_names:
        if sht_name in wb.sheetnames:
            wb[sht_name].sheet_view.showGridLines = True
...
def populate_percentile_sheet(
    ...
    _apply_percentile_date_formats(ws, 5, data_rows)
    apply_full_grid(ws, ws.max_row, ws.max_column)
...
def process_excel(source_path: str, template_path: str) -> str:
    ...
    fill_monthly_claim_summary_columns(ws, DATA_START, last_row, cols, claim_sums)

    # 預先收集 candidates，供 kpi_marks 與 KPI 彙總共用，避免重複掃描
    hba_candidates = _collect_hba_candidates(ws, cols, DATA_START, last_row)
    ldl_candidates = _collect_ldl_candidates(ws, cols, DATA_START, last_row)

    kpi_marks = collect_kpi_mark_sets(ws, cols, DATA_START, last_row,
                                      hba_candidates=hba_candidates,
                                      ldl_candidates=ldl_candidates)
    _compute_all_derived(ws, cols, meta, DATA_START, last_row, now, kpi_marks)

    apply_date_format(ws, cols, DATA_START, last_row)

    calc_hba_kpi_ay_az(ws, cols, DATA_START, last_row, hba_candidates=hba_candidates)
    calc_ldl_percentiles(ws, cols, DATA_START, last_row, ldl_candidates=ldl_candidates)
    populate_percentile_sheet(wb_tpl, ws, cols, DATA_START, last_row)

    # 主表保留模板原始線條，不再重畫 border；只對寫入資料的分頁開啟原生格線
    _enable_gridlines(wb_tpl, (Rules.SHEET_TARGET, PERCENTILE_SHEET_NAME))
