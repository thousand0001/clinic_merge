...
class Rules:
    ...
    # 模板設定（0327）
    TEMPLATE_NAME: str = "選會員樣板0331.xlsx"
    SHEET_TARGET: str = "會員總表"
    DATA_START_ROW: int = 3

    # 月份申請統計輸出欄位（0325）
    # L=114件數, M=114實際申報總額, N=115件數, O=115實際申報總額
    COL_114_COUNT: str = "L"
...
def fill_monthly_claim_summary_columns(
    ...
    col_s    = cols.get("r_amount_114")           # M：114實際申報總額
    col_t    = cols.get("s_amount_115")           # O：115實際申報總額
...
def populate_doctor_sheet(
    ...
    vertical_center_wrap = Alignment(vertical="center", wrap_text=True)
    vertical_center_no_wrap = Alignment(vertical="center")
        ...
        for col_letter, kind in (
            ...
            if dt is not None:
                if _doctor_screening_is_overdue(kind, dt=dt, age=age_num, sex=sex_val, today=today):
                    ...
                    cell.alignment = vertical_center_wrap
                else:
                    cell.alignment = vertical_center_no_wrap
                continue
