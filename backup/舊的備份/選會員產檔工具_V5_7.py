...
def calc_score(
    *,
    ...
    breakdown = (
        f"1.年齡{age_score}分，"
        f"2.預防保健{prevention_score}分，"
        f"3.檢查{exam_score}分，"
        f"4.固定就診次數{visit_score}分"
    )
...
def collect_kpi_mark_sets(
    ...
    ldl_k = int(math.ceil(Rules.HBA_TARGET_PERCENT * len(ldl_sorted))) if ldl_sorted else 0
...
def _compute_all_derived(
    ...
    for rr in range(data_start, last_row + 1):
        ...
        fit_dt   = _get_dt("fit")
        hep_dt   = _get_dt("hep")
        hba_dt   = _get_dt("hba_dt")   # 你的模板是 AD
        ldl_dt   = _get_dt("ldl_dt")   # 你的模板是 AF
