# -*- coding: utf-8 -*-
"""
選會員產檔工具（結構化單檔版）— V5.14（0313樣板／主次診斷修正版）
...
- 新增：
  ...
  - AC 主次診斷：若原始檔有「主次診斷」sheet，依診斷碼填入 DM / CKD / DM+CKD
- KPI
  - HbA1c：AY8（<=7 的比例）
...
def _build_main_sub_dx_map(sh_dx: Any) -> Dict[str, str]:
    ...
    return {pid: classify_main_sub_dx(codes) for pid, codes in id_to_codes.items() if classify_main_sub_dx(codes)}
...
def _fill_main_sub_dx(
    ...
    target_col = cols.get("main_sub_dx")
    if sh_dx is None or not target_col:
        return

    id_to_label = _build_main_sub_dx_map(sh_dx)
    for pid, rows in id_to_rows.items():
        label = id_to_label.get(normalize_id(pid), "")
        if not label:
            continue
        for rr in rows:
            safe_set(ws, rr, target_col, label)
...
def populate_doctor_sheet(
    ...
    """將會員指標資料 copy 到「醫生看」sheet，從 row 3 開始寫入"""
    if DOCTOR_SHEET_NAME not in wb_tpl.sheetnames:
        return  # 模板沒有此 sheet，略過
    ...
    # 建立來源欄對照：直接用 cols dict（已含 COLUMN_SHIFT）
    # 不在 cols 的欄（如生日G、電話K等基本欄）用原始欄號（AC前無位移）
    from openpyxl.utils import column_index_from_string
    ...
    for src_letter, _ in _DOCTOR_COL_MAP:
        key = _key_by_letter.get(src_letter)
        if key and cols.get(key):
            src_col_map[src_letter] = cols[key]
        else:
            # 基本欄（AC 前）直接用欄號
            src_col_map[src_letter] = column_index_from_string(src_letter)

    # 清空醫生看舊資料（從 row 3 開始）
    for r in range(DOC_DATA_START, ws_doc.max_row + 1):
        for c in range(1, ws_doc.max_column + 1):
            ws_doc.cell(r, c).value = None

    # 逐列 copy
    for src_row in range(data_start, last_row + 1):
        dst_row = DOC_DATA_START + (src_row - data_start)
        for src_letter, dst_letter in _DOCTOR_COL_MAP:
            src_c = src_col_map.get(src_letter)
            dst_c = column_index_from_string(dst_letter)
            if src_c:
                ws_doc.cell(dst_row, dst_c).value = ws_main.cell(src_row, src_c).value

    ws_doc.sheet_view.showGridLines = True
    print(f"醫生看 sheet 已寫入 {last_row - data_start + 1} 列")
...
def collect_all_members(wb_src) -> Dict[str, Dict[str, Any]]:
    ...
    for sname in month_sheets:
        sh = wb_src[sname]
        header_row = _find_header_row_contains_any(sh, [id_aliases], search_rows=5)
        if header_row is None:
            header_row = 1
        hmap = build_header_map(sh, header_row)
        id_col_cand = find_column_exact(hmap, id_aliases)
        id_col = find_id_col_by_content(sh, header_row, id_col_cand)
        name_col = find_column_exact(hmap, ["姓名"])
        if id_col is None:
            continue
        for r in range(header_row + 1, sh.max_row + 1):
