...
class Rules:
    ...
    COL_IS_115X: str = "AZ"
    COL_NOTE: str = "AW"
    FLAG_VALUE: str = "✔"   # 旗標欄打勾值（is_114 / is_self_select / is_115x）
...
def _cache_row_styles(ws, src_row: int, start_col: int, end_col: int) -> dict:
    """
    預先快取模板列的樣式，供大量寫入時重複套用，
    避免每列都重複讀取 src_row 的 Cell 物件（openpyxl 存取成本高）。
    回傳 {col: {屬性dict}} 的快取。
    """
    cache = {}
...
def _new_member_record() -> Dict[str, Any]:
    return {
        "name": None,
        "bday": None,
        "e_code": None,
...
def _merge_member_record(dst: Dict[str, Any], src: Dict[str, Any]) -> None:
    """以欄位為單位合併；來源有值才覆蓋，空值不清掉既有資料。"""
...
def _extract_member_partial_map(sheet, id_aliases: List[str], search_rows: int = 10) -> Dict[str, Dict[str, Any]]:
    ...
    bday_col = find_column_exact(hmap, ["會員生日", "生日", "出生日期", "生日日期"])
    phone_cols = [
    ...
    for r in range(header_row + 1, sheet.max_row + 1):
        ...
        contact = pick_contact_from_values([sheet.cell(r, c).value for c in phone_cols]) if phone_cols else ContactInfo()
        address = normalize_text(sheet.cell(r, addr_col).value) if addr_col else ""
        partial = {
            "name": normalize_text(sheet.cell(r, name_col).value) if name_col else None,
            "bday": parse_date(sheet.cell(r, bday_col).value) if bday_col else None,
            "phone": contact.phone,
        ...
        if pid not in result:
            result[pid] = _new_member_record()
        _merge_member_record(result[pid], partial)
...
def collect_all_members(wb_src) -> Dict[str, Dict[str, Any]]:
    ...
    月份分頁同一人以最後月份為主。
    回傳 {id: {name, bday, e_code, abc, phone, mobile, address, dmk_raw}}
    """
    ...
    for sname in month_sheets:
        ...
        for pid, partial in partial_map.items():
            members.setdefault(pid, _new_member_record())
            _merge_member_record(members[pid], partial)
    ...
    if "會員名單" in snames:
        ...
        if id_col:
            for r in range(header_row + 1, sh.max_row + 1):
                ...
                if not pid or not is_valid_tw_id(pid):
                    continue
                contact = pick_contact_from_values([
                    sh.cell(r, c).value for c in phone_cols
                ])
                address = normalize_text(sh.cell(r, addr_col).value) if addr_col else ""
                members.setdefault(pid, _new_member_record())
                _merge_member_record(members[pid], {
...
def _fill_extra_flags(
    ...
    for pid, rows in id_to_rows.items():
        p4p = p4p_map.get(pid, {})
        is_114       = Rules.FLAG_VALUE if pid in ascvd_ids      else None
        is_self      = Rules.FLAG_VALUE if pid in self_select_ids else None
        is_115x      = Rules.FLAG_VALUE if pid in x115_ids        else None
...
def _fill_member_basic(
    ...
    # 預快取模板列樣式，避免每列重複讀取（3000+ 列時效能差異顯著）
    _max_col = ws.max_column
    _style_cache = _cache_row_styles(ws, data_start, 1, _max_col)

    for pid, info in all_members.items():
        if out_r != data_start:  # 第一列本身即模板列，不需複製
            _apply_cached_style(ws, out_r, _style_cache, 1, _max_col)
        ...
        safe_set(ws, out_r, cols.get("tel"),      contact.phone or None)
        safe_set(ws, out_r, cols.get("mobile"),   contact.mobile or None)
        ...
        safe_set(ws, out_r, cols.get("address_hidden"), contact.address or None)
...
def load_source(source_path: str) -> SourceContext:
    log_progress(f"載入原始檔：{os.path.basename(source_path)}")
...
def load_template(template_path: str) -> TemplateContext:
    log_progress(f"載入模板：{os.path.basename(template_path)}")
...
def fill_external_data(source_ctx: SourceContext, template_ctx: TemplateContext, runtime_ctx: RuntimeContext) -> None:
    log_progress("回填外部資料：ASCVD / 主次診斷 / 篩檢 / HealthCase / 月份統計")
    ws = template_ctx.ws
...
def finalize_and_save(source_path: str, template_ctx: TemplateContext, now_dt: datetime.datetime) -> str:
    ...
    log_progress(f"完成輸出：{os.path.basename(out_path)}")
    return out_path
...
def process_excel(source_path: str, template_path: str) -> str:
    ...
    if runtime_ctx.last_row < template_ctx.data_start:
        log_progress("沒有可輸出的會員資料，直接輸出空白結果")
