...
# ============================================================
# KPI 儲存格地址（0320 樣板固定位置，直接寫死）
# BG = HbA1c 主, BH = HbA1c 73.8% 目標
# BJ = LDL 主,   BK = LDL 73.8% 目標
# ============================================================
SUMMARY_CELLS = {
    "hba_ratio_main":      "BG8",
    "hba_fraction_main":   "BG9",
    "hba_cutoff_label":    "BH7",
    "hba_ratio_target":    "BH8",
    "hba_fraction_target": "BH9",
    "ldl_ratio_main":      "BJ8",
    "ldl_fraction_main":   "BJ9",
    "ldl_cutoff_label":    "BK7",
    "ldl_ratio_target":    "BK8",
    "ldl_fraction_target": "BK9",
...
def _detect_output_cols(ws, header_row: int, max_scan_row: int) -> Dict[str, Optional[int]]:
    ...
    return {
        ...
        "breakdown": (kw(ws, header_row, ["分數說明"])
                        ...
                        or column_index_from_string("BL")),  # 0320 固定位置
...
def _detect_kpi_col(
    ws, max_scan_row: int, keywords: List[str],
    exclude_73: bool = False, fallback: str = ""
) -> Optional[int]:
    """
    找 KPI 百分位標記欄：
    - exclude_73=True  → 找「含所有 keywords 但不含73.8%」的欄（主欄 BG/BJ）
    - exclude_73=False → 找不到時直接用 fallback 固定欄位
    - fallback → 找不到時用固定欄位字母
    """
    keys = [k.replace(" ", "").replace("\n", "") for k in keywords if k]
    for c in range(1, ws.max_column + 1):
        blob = "".join(
        ...
        if all(k in blob for k in keys):
            if exclude_73 and "73.8" in blob:
                continue
            return c
    return column_index_from_string(fallback) if fallback else None
...
def detect_template_columns(ws, data_start: int) -> Dict[str, Optional[int]]:
    ...
    # KPI 標記欄（0320 樣板固定位置）：
    # BG=HbA1c主(<=7), BH=HbA1c 73.8%目標, BJ=LDL主(<=100), BK=LDL 73.8%目標
    # ay/bb 主欄可從標題偵測；az/bc 目標欄標題過短無法區分，直接用相鄰欄（主欄+1）
    _ay = _detect_kpi_col(ws, max_scan_row, ["HbA1c", "百分位"], exclude_73=True, fallback="BG")
    _bb = _detect_kpi_col(ws, max_scan_row, ["LDL",   "百分位"], exclude_73=True, fallback="BJ")
    cols["ay_mark"] = _ay
    cols["az_mark"] = (_ay + 1) if _ay else column_index_from_string("BH")
    cols["bb_mark"] = _bb
    cols["bc_mark"] = (_bb + 1) if _bb else column_index_from_string("BK")
...
def _build_src_col_map(
    ...
    建立來源欄字母 → 實際欄號的對照表。
    優先用 cols dict（已含欄位偵測結果），找不到則直接用欄字母轉欄號。
    """
