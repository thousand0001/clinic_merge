...
class Rules:
    ...
    # 模板設定（0327）
    TEMPLATE_NAME: str = "選會員模板0407V1.xlsx"
...
def collect_monthly_claim_summaries(wb_src) -> Tuple[Dict[str, Dict[str, float]], List[int]]:
    """
    掃描 R11440 類明細表，
    依 ID 彙總 R11440 明細格式：
      - 日期欄 -> 最後就診日
...
def _copy_sheet_rows(
    ...
    needed_src_cols = sorted({src_c for src_c, _ in _col_pairs if src_c})
    if filter_ids is not None and id_main_col:
        needed_src_cols = sorted(set(needed_src_cols) | {id_main_col})
    dst_row = dst_data_start
    for src_row in range(data_start, last_row + 1):
        row_cache = {col_num: ws_main.cell(src_row, col_num).value for col_num in needed_src_cols}
        # 過濾 ID
        if filter_ids is not None:
            pid = normalize_id(row_cache.get(id_main_col)) if id_main_col else ""
            if pid not in filter_ids:
                continue

        for src_c, dst_c in _col_pairs:
            if src_c:
                val = row_cache.get(src_c)
...
def _finalize_doctor_sheet_alignment(ws_doc) -> None:
    ...
    left_cols = {column_index_from_string(col) for col in ("G", "AN", "AO")}
    wrap_if_text_cols = {column_index_from_string(col) for col in ("P", "Q", "R", "S", "T", "Y")}
    wrap_if_multiline_cols = {column_index_from_string(col) for col in ("U", "W")}
    wrap_if_status_cols = {column_index_from_string(col) for col in ("V", "X")}
    align_left = _get_alignment("left", wrap_text=True)
    align_center_wrap = _get_alignment("center", wrap_text=True)
    align_center_plain = _get_alignment("center", wrap_text=False)
    for row in ws_doc.iter_rows(min_row=4, max_row=ws_doc.max_row, min_col=1, max_col=end_col):
        for cell in row:
            if cell.value in (None, ""):
                continue
            if cell.column in left_cols:
                cell.alignment = align_left
            elif cell.column in wrap_if_text_cols:
                cell.alignment = align_center_wrap
            elif cell.column in wrap_if_multiline_cols:
                text = str(cell.value)
                cell.alignment = align_center_wrap if "\n" in text else align_center_plain
            elif cell.column in wrap_if_status_cols:
                text = str(cell.value)
                cell.alignment = align_center_wrap if text in ("待受檢", "不需受檢") else align_center_plain
            else:
...
def populate_doctor_sheet(
    ...
    doc_col_idx = {col: column_index_from_string(col) for col in (
        "D", "H", "I", "AJ", "AK", "AL", "AM",
        "P", "Q", "R", "S", "T",
        "U", "V", "W", "X", "Y", "Z",
    )}
    ...
    lab_date_pairs = (("V", "hba"), ("X", "ldl"))
    lab_value_pairs = (("U", "hba", hba_pass_col), ("W", "ldl", ldl_pass_col))
    for offset, src_row in enumerate(range(data_start, last_row + 1), start=4):
        def _main_val(col_num: Optional[int]) -> Any:
            return ws_main.cell(src_row, col_num).value if col_num else None

        def _doc_cell(col_letter: str):
            return ws_doc.cell(offset, doc_col_idx[col_letter])

        visit_score = fee_score = exam_score = prevention_score = None
        breakdown_value = _main_val(breakdown_col)
        if breakdown_col:
            visit_score, fee_score, exam_score, prevention_score = _extract_score_components(
                breakdown_value
            )
        raw_dmk_value = _main_val(cols["dmk_code"])
        e_code = parse_disease_code(raw_dmk_value)
        ascvd_raw = _main_val(cols["ascvd"])
        age_val = _main_val(age_col)
        sex_val = normalize_text(_main_val(sex_col))
        abc_val = normalize_text(_main_val(abc_col))
        disease_text_val = normalize_text(_main_val(disease_text_col))
        leak_item_val = normalize_text(_main_val(leak_item_col))
        _doc_cell("D").value = age_val
        _doc_cell("H").value = _doctor_dmk_display(e_code, raw_dmk_value)
        _doc_cell("I").value = _doctor_ascvd_display(ascvd_raw)
        _doc_cell("AJ").value = visit_score
        _doc_cell("AK").value = fee_score
        _doc_cell("AL").value = exam_score
        _doc_cell("AM").value = prevention_score

        age_num = _safe_int(age_val)
        for col_letter, kind in screening_pairs:
            cell = _doc_cell(col_letter)
        ...
        for col_letter in ("P", "Q", "R", "S", "T"):
            _doc_cell(col_letter).border = _BORDER_THIN_GRAY

        for col_letter, kind in lab_date_pairs:
            cell = _doc_cell(col_letter)
            dt = parse_date(cell.value)
        ...
        hba_pass_mark = normalize_text(_main_val(hba_pass_col))
        ldl_pass_mark = normalize_text(_main_val(ldl_pass_col))
        pass_mark_map = {"U": hba_pass_mark, "W": ldl_pass_mark}

        for col_letter, kind, pass_col in lab_value_pairs:
            cell = _doc_cell(col_letter)
            cell.border = _BORDER_THIN_GRAY
            cell.font = copy(cell.font)
            date_col_letter = "V" if col_letter == "U" else "X"
            date_cell_value = _doc_cell(date_col_letter).value
            ...
            pass_mark = pass_mark_map.get(col_letter, "") if pass_col else ""
            if pass_mark != "✔":
                cell.fill = _FILL_NONE
        ...
        uacr_cell = _doc_cell("Y")
        uacr_cell.font = copy(uacr_cell.font)
        uacr_val_num = parse_float(uacr_cell.value)
        uacr_dt = parse_date(_doc_cell("Z").value)
        uacr_pass_mark = normalize_text(_main_val(uacr_pass_col))
...
def populate_self_select_sheet(
    ...
    self_select_ids: set = {
        normalize_id(row_vals[0])
        for row_vals in sh_self_select.iter_rows(
            min_row=header_row + 1,
            max_row=sh_self_select.max_row,
            min_col=id_col,
            max_col=id_col,
            values_only=True,
        )
        if row_vals and is_valid_tw_id(row_vals[0])
    }
...
def _finalize_main_sheet_alignment(ws, data_start: int, last_row: int) -> None:
    ...
    align_left = _get_alignment("left", wrap_text=True)
    align_center = _get_alignment("center", wrap_text=True)
    for row in ws.iter_rows(min_row=data_start, max_row=last_row, min_col=1, max_col=end_col):
        for cell in row:
            if cell.value in (None, ""):
                continue
            if cell.column in _LEFT_COLS:
                cell.alignment = align_left
            else:
...
def _finalize_self_select_sheet_alignment(ws) -> None:
    ...
    align_center = _get_alignment("center", wrap_text=True)
    for row in ws.iter_rows(min_row=3, max_row=ws.max_row, min_col=1, max_col=end_col):
        for cell in row:
            if cell.value in (None, ""):
                continue
            cell.alignment = align_center
...
def _make_percentile_record(
    ws_main,
    row: int,
    cols: Dict[str, Optional[int]],
    metric: str,
    name_fill: str,
    row_cache: Optional[Dict[int, Any]] = None,
) -> Dict[str, Any]:
    value_col = cols["ldl"] if metric == "ldl" else cols["hba"]
    date_col = cols["ldl_dt"] if metric == "ldl" else cols["hba_dt"]

    def _val(col_num: Optional[int]) -> Any:
        if not col_num:
            return None
        if row_cache is not None:
            return row_cache.get(col_num)
        return ws_main.cell(row, col_num).value
    ...
    return {
        ...
        "bday": _val(cols["bday"]),
        "id": _val(cols["id"]),
        "score": _val(cols["score"]),
        "note": _val(cols["note"]),
        "value": _val(value_col),
        "last_dt": _val(date_col),
...
def _collect_percentile_records(
    ...
    needed_cols = {
        c for c in (
            c_bb, c_bc, c_ay, c_az,
            cols.get("name"), cols.get("bday"), cols.get("id"), cols.get("score"), cols.get("note"),
            cols.get("hba"), cols.get("hba_dt"), cols.get("ldl"), cols.get("ldl_dt"),
            cols.get("au"), cols.get("av"), cols.get("aw"),
        ) if c
    }

    for row in range(data_start, last_row + 1):
        row_cache = {col_num: ws_main.cell(row, col_num).value for col_num in needed_cols}

        if c_bb and normalize_text(row_cache.get(c_bb)) == "✔":
            ldl_primary.append(row)
        elif c_bc and normalize_text(row_cache.get(c_bc)) == "✔":
            ldl_secondary.append(row)

        if c_ay and normalize_text(row_cache.get(c_ay)) == "✔":
            hba_primary.append(row)
        elif c_az and normalize_text(row_cache.get(c_az)) == "✔":
            hba_secondary.append(row)

    ldl_records = (
        [_make_percentile_record(
            ws_main,
            r,
            cols,
            "ldl",
            "pink",
            {col_num: ws_main.cell(r, col_num).value for col_num in needed_cols},
        ) for r in ldl_primary] +
        [_make_percentile_record(
            ws_main,
            r,
            cols,
...
def _finalize_percentile_sheet_alignment(ws) -> None:
    ...
    while end_row >= 5:
        if any(ws.cell(end_row, col).value not in (None, "") for col in range(1, 25)):
            break
        end_row -= 1
    left_offsets = set(PERCENTILE_LEFT_OFFSETS)
    align_left = _get_alignment("left", wrap_text=True)
    align_center = _get_alignment("center", wrap_text=True)
    for row in ws.iter_rows(min_row=5, max_row=end_row, min_col=1, max_col=24):
        for cell in row:
            if cell.value in (None, ""):
                continue
            relative_offset = (cell.column - 1) % 13
            if relative_offset in left_offsets:
                cell.alignment = align_left
            else:
                cell.alignment = align_center
    ...
    for pid, rows in id_to_rows.items():
        ...
        if in_member:
            ...
            if e_code in (DiseaseCode.DM, DiseaseCode.CKD, DiseaseCode.DKD) or ascvd in (AscvdCategory.A, AscvdCategory.B):
                ...
                base = "B"
        elif in_self_select and in_115x:
            base = "E1/E2"
        elif in_self_select:
            base = "E1"
        elif in_115x:
        ...
        if in_self_select and in_115x and base in ("A", "B"):
            value = f"{base}/E1/E2"
        elif in_self_select and base in ("A", "B"):
            value = f"{base}/E1"
        elif in_115x and base in ("A", "B"):
            value = f"{base}/E2"
        else:
            value = base
...
def _compute_all_derived(
    ...
    for rr in range(data_start, last_row + 1):
        ...
        def _row_val(col_num: Optional[int]) -> Any:
            if not col_num:
                return None
            if col_num not in row_cache:
                row_cache[col_num] = ws.cell(rr, col_num).value
            return row_cache[col_num]

        def _row_dt(col_num: Optional[int]) -> Optional[datetime.date]:
            return parse_date(_row_val(col_num))

        dmk_raw   = _row_val(_c_dmk)
        ascvd_raw = _row_val(_c_ascvd)
        e_code    = parse_disease_code(dmk_raw)
        ascvd     = parse_ascvd(ascvd_raw)

        m   = meta.get(rr, MemberMeta(row=rr))
        age = m.age if isinstance(m.age, int) else -1
        sex = normalize_text(_row_val(_c_sex))

        adult_dt = _row_dt(_c_adult)
        pap_dt   = _row_dt(_c_pap)
        flu_dt   = _row_dt(_c_flu)
        fit_dt   = _row_dt(_c_fit)
        hep_dt   = _row_dt(_c_hep)
        hba_dt   = _row_dt(_c_hba_dt)
        ldl_dt   = _row_dt(_c_ldl_dt)

        hba_val = _row_val(_c_hba)
        ldl_val = _row_val(_c_ldl)
        uacr_val = _row_val(_c_uacr)
        ...
        visit_count_114 = _row_val(_c_m114)
        visit_count_115 = _row_val(_c_n115)
        claim_amount_114 = _row_val(_c_r114)
        claim_amount_115 = _row_val(_c_s115)
        fee_score_amount_114 = claim_amount_114
        fee_score_amount_115 = claim_amount_115
        if claim_sums is not None:
            pid = normalize_text(_row_val(_c_id)).upper()
            data = claim_sums.get(pid)
            if data:
                v114a_total = data.get("114_amt_total", 0.0)
                v115a = data.get("115_amt", 0.0)
...
def prepare_template_layout(ws) -> None:
    ...
    ws["BC1"] = ws["BC1"].value or "115年1-4月就診次數"
    ws["BF1"] = ws["BF1"].value or "地址"
    ws["BG1"] = ws["BG1"].value or "114年1-4月就診次數"
    for addr in ("AZ2", "BA2", "BB2", "BC2", "BF2", "BG2"):
...
def load_source(source_path: str) -> SourceContext:
    ...
    claim_sums, claim_months_115 = collect_monthly_claim_summaries(wb_src)
...
def main() -> None:
    ...
    status_win.title("處理中")
    status_win.resizable(False, False)
    status_win.protocol("WM_DELETE_WINDOW", lambda: None)

    ttk.Label(status_win, text="正在處理 Excel，請稍候...").pack(padx=20, pady=(16, 8))
    status_var = tk.StringVar(value="已選擇資料夾，準備開始。")
    ttk.Label(status_win, textvariable=status_var, wraplength=320, justify="left").pack(padx=20, pady=(0, 8))
    ttk.Label(status_win, text="詳細進度仍會同步輸出到終端機。").pack(padx=20, pady=(0, 12))
    progress = ttk.Progressbar(status_win, mode="indeterminate", length=260)
    ...
    x = (status_win.winfo_screenwidth() - width) // 2
    y = (status_win.winfo_screenheight() - height) // 2
    ...
    def worker() -> None:
        ...
        try:
            ...
            traceback.print_exc()
            result_queue.put(("error", f"未預期錯誤：{e}"))
        finally:
            globals()["_log"] = original_log
    ...
    def poll_result() -> None:
        ...
        if kind == "success":
            ...
            open_file_cross_platform(payload)
        else:
