...
def fill_monthly_claim_summary_columns(
    ...
    for rr in range(data_start, last_row + 1):
        ...
        ws.cell(rr, col_s).value = _to_excel_number(v114a_avg) if v114a != 0 else None
        ws.cell(rr, col_t).value = _to_excel_number(v115a_avg) if v115a != 0 else None
        if col_s_total:
            ws.cell(rr, col_s_total).value = _to_excel_number(v114a) if v114a != 0 else None
        if col_t_total:
            ws.cell(rr, col_t_total).value = _to_excel_number(v115a) if v115a != 0 else None
...
def _detect_screening_cols(ws, header_row: int) -> Dict[str, Optional[int]]:
    ...
    0322 樣板固定位置：Q=成人, R=子宮, S=老人流感, T=糞便, U=肝炎
    """
    ...
    return {
        "adult": _find_screening(["成人", "健檢"], "Q"),
        "pap":   _find_screening(["子宮", "抹片"], "R"),
        "flu":   _find_screening(["老人", "流感"], "S"),
        "fit":   _find_screening(["糞便", "潛血"], "T"),
        "hep":   _find_screening(["BC肝炎"],       "U"),
    }
...
def _detect_output_cols(ws, header_row: int, max_scan_row: int) -> Dict[str, Optional[int]]:
    ...
    return {
        ...
        "score": (kw(ws, header_row, ["分數"])
                  or column_index_from_string("AU")),
        "breakdown": (kw(ws, header_row, ["分數說明"])
                        or kw_any(ws, max_scan_row, ["分數說明"])
                        or column_index_from_string("AV")),
        "note":  (kw(ws, header_row, ["備註"])
                  or column_index_from_string(Rules.COL_NOTE)),
        ...
        "ak":    (kw_any(ws, max_scan_row, ["打勾"])
                  ...
                  or column_index_from_string("AJ")),
        # AX：漏檢項目
...
def detect_template_columns(ws, data_start: int) -> Dict[str, Optional[int]]:
    ...
    cols["r_amount_114_total"] = column_index_from_string(Rules.COL_114_AMOUNT_TOTAL)
    cols["s_amount_115_total"] = column_index_from_string(Rules.COL_115_AMOUNT_TOTAL)
    cols["is_114"]         = column_index_from_string(Rules.COL_IS_114)
    cols["is_self_select"] = column_index_from_string(Rules.COL_IS_SELF_SELECT)
    cols["is_115x"]        = column_index_from_string(Rules.COL_IS_115X)
    # KPI 標記欄（0320 樣板固定位置）：
    # BG=HbA1c主(<=7), BH=HbA1c 73.8%目標, BJ=LDL主(<=100), BK=LDL 73.8%目標
    # ay/bb 主欄可從標題偵測；az/bc 目標欄標題過短無法區分，直接用相鄰欄（主欄+1）
    cols["ay_mark"] = column_index_from_string("AQ")
    cols["az_mark"] = column_index_from_string("AR")
    cols["bb_mark"] = column_index_from_string("AS")
    cols["bc_mark"] = column_index_from_string("AT")
...
_LETTER_TO_COL_KEY: Dict[str, str] = {
    ...
    "BA": "score",
    "BB": "m_count_114_full",
    "BC": "r_amount_114_total",
    "BD": "s_amount_115_total",
}
...
_DOCTOR_COL_MAP: List[Tuple[str, str]] = [
    ...
    ("L",  "J"),   # 次數 114
    ("N",  "K"),   # 次數 115
    ("BC", "L"),   # 申報金額 114總額（隱藏輔助欄）
    ("BD", "M"),   # 申報金額 115總額（隱藏輔助欄）
    ("Q",  "N"),   # 成人預防保健日期
    ("R",  "O"),   # 子宮頸抹片日期
    ("S",  "P"),   # 老人流感日期
    ("T",  "Q"),   # 糞便潛血日期
...
def prepare_template_layout(ws) -> None:
    ...
    for addr in ("AX2", "AY2", "AZ2", "BB2", "BC2", "BD2"):
