...
class Rules:
    ...
    # 0313 樣板：在 AC 前新增 1 欄；此值由模板偵測後自動決定
    COLUMN_SHIFT: int = 0
...
ID_PATTERNS = (
    re.compile(r"^[A-Za-z][0-9]{9}$"),
    re.compile(r"^[A-Za-z]{2}[0-9]{8}$"),
)


def is_valid_person_id(v: Any) -> bool:
    s = normalize_text(v).upper()
    return any(p.fullmatch(s) for p in ID_PATTERNS)


def find_header_and_first_data_row_by_id(
    sheet,
    header_aliases: List[str],
    search_rows: int = 50,
) -> Tuple[int, int, int]:
    alias_set = {normalize_header(x) for x in header_aliases}
    max_r = min(search_rows, sheet.max_row)

    for r in range(1, max_r + 1):
        for c in range(1, sheet.max_column + 1):
            head = normalize_header(sheet.cell(r, c).value)
            if head not in alias_set:
                continue
            for rr in range(r + 1, sheet.max_row + 1):
                if is_valid_person_id(sheet.cell(rr, c).value):
                    return r, c, rr

    raise ValueError(
        f"工作表「{sheet.title}」找不到 ID 標題欄或內容起始列"
    )
...
def extract_icd_codes_from_cell(v: Any) -> List[str]:
    ...
    found = re.findall(r"\b[A-Z]\d{1,2}(?:\.\d+)?\b", s)
    out: List[str] = []
    seen = set()
    for raw in found:
...
def is_ckd_icd_code(code: str) -> bool:
    return normalize_icd_code(code) in CKD_CODES
...
def _build_main_sub_dx_map(sh_dx: Any) -> Dict[str, str]:
    ...
    dx_header_aliases = [
        "主診斷", "主診斷碼", "主診", "次診斷", "次診斷碼", "次診",
        "診斷碼", "診斷代碼", "ICD", "ICD10", "疾病碼",
    ]
    header_row = _find_header_row_contains_any(sh_dx, [id_aliases, dx_header_aliases], search_rows=20)
    if header_row is None:
        raise ValueError("原始檔「主次診斷」找不到 ID / 診斷碼 欄位")
    ...
    for hdr, c in hmap.items():
        if c == id_col:
            continue
        if any(alias.lower() in hdr.lower() for alias in ["主診斷", "主診斷碼", "主診", "次診斷", "次診斷碼", "次診", "診斷碼", "診斷代碼", "icd", "icd10", "疾病碼"]):
            dx_cols.append(c)
...
def _detect_output_cols(ws, header_row: int, max_scan_row: int) -> Dict[str, Optional[int]]:
    ...
    return {
        ...
        "breakdown": (kw(ws, header_row, ["分數說明"])
                        ...
                        or column_index_from_string(tpl_col("BD"))),
...
def _fill_member_basic(
    ...
    member_header_row, _, member_first_data_row = find_header_and_first_data_row_by_id(
        sh_member,
        ["會員身份証", "會員身份證", "會員身分證", "身份證號", "身分證號", "ID"],
        search_rows=50,
    )
    mmap = build_header_map(sh_member, member_header_row)

    m_name = find_column_exact(mmap, ["會員姓名", "姓名"])
    m_id   = find_column_exact(mmap, ["會員身份証", "會員身份證", "會員身分證", "身份證號", "身分證號", "ID"])
    m_bday = find_column_exact(mmap, ["會員生日", "生日", "BIRTHDAY"])
    m_tel  = find_column_exact(mmap, ["電話", "手機", "手機號碼"])
    m_abc  = find_column_exact(mmap, ["會員別", "A/B/C"])
    m_dmk  = find_column_exact(mmap, ["疾病樣態", "DM/CKD/DKD"])
    m_cnt  = find_column_exact(mmap, ["就診次數", "次數"])
    ...
    for r in range(member_first_data_row, sh_member.max_row + 1):
        ...
        safe_set(ws, out_r, cols.get("id"),       sid_s)
...
def _fill_ascvd(
    ...
    ascvd_header_row, _, ascvd_first_data_row = find_header_and_first_data_row_by_id(
        sh_ascvd,
        ["ID", "id", "身分證號", "身份證號"],
        search_rows=50,
    )
    amap  = build_header_map(sh_ascvd, ascvd_header_row)
    a_id  = find_column_exact(amap, ["ID", "id", "身分證號", "身份證號"])
    a_asc = find_column_exact(amap, ["ASCVD", "ascvd"])
    if a_id is None or a_asc is None:
        raise ValueError("原始檔「ascvd」找不到 ID / ASCVD 欄位")

    for r in range(ascvd_first_data_row, sh_ascvd.max_row + 1):
        pid = sh_ascvd.cell(r, a_id).value
        val = sh_ascvd.cell(r, a_asc).value
        if not is_valid_person_id(pid):
            continue
        if val is None or str(val).strip() in ("",):
            continue

        pid_s = normalize_text(pid).upper()
        rows = id_to_rows.get(pid_s)
        if not rows:
            continue
...
def _fill_screening(
    ...
    header_row, _, first_data_row = find_header_and_first_data_row_by_id(
        sheet,
        ["ID", "身分證號", "身份證號"],
        search_rows=20,
    )
    ...
    for rr in range(first_data_row, sheet.max_row + 1):
        pid = sheet.cell(rr, sid_col).value
        dt  = parse_date(sheet.cell(rr, dt_col).value)
        if not is_valid_person_id(pid) or dt is None:
            continue
...
def load_source(source_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "sh_main_sub_dx": wb_src["主次診斷"] if "主次診斷" in wb_src.sheetnames else None,
        "screening_sheets": {
            "adult": wb_src["成人健檢"],
            "pap": wb_src["子宮抹片"],
            "flu": wb_src["老人流感"],
