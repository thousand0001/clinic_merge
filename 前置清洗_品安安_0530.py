...
def _postprocess_member_key(name: Any, bday: Any) -> Optional[Tuple[str, str]]:
    text = GENERIC.normalize_text(name)
    parsed = GENERIC.parse_date(bday)
    if not text or not parsed:
        return None
    return (text.replace(" ", "").upper(), parsed.isoformat())


def _rec_key(rec: Any) -> Optional[Tuple[str, str]]:
    return _postprocess_member_key(getattr(rec, "name", ""), getattr(rec, "bday", None))


def _effective_115_months(source_dir: Path, records: Sequence[Any]) -> int:
    months = set()
    for path in source_dir.rglob("*"):
        if not path.is_file():
            continue
        match = re.search(r"(?<!\d)115(\d{2})(?!\d)", path.stem)
        if match:
            months.add(int(match.group(1)))
    if not months:
        for rec in records:
            if getattr(rec, "count_115", 0) or getattr(rec, "amount_115", 0):
                months.add(1)
    return max(len(months), 1)


def _record_has_month_data(rec: Any) -> bool:
    return bool(
        getattr(rec, "count_114", 0)
        or getattr(rec, "amount_114", 0)
        or getattr(rec, "count_115", 0)
        or getattr(rec, "amount_115", 0)
    )
...
def _id_rows(ws, id_col: int, start_row: int = 3) -> Dict[str, int]:
    out: Dict[str, int] = {}
    for row in range(start_row, ws.max_row + 1):
        pid = GENERIC.normalize_id(ws.cell(row, id_col).value)
        if pid and pid not in out:
            out[pid] = row
    return out


def _key_rows(ws, name_col: int, bday_col: int, start_row: int = 3) -> Dict[Tuple[str, str], int]:
    out: Dict[Tuple[str, str], int] = {}
    for row in range(start_row, ws.max_row + 1):
        key = _postprocess_member_key(ws.cell(row, name_col).value, ws.cell(row, bday_col).value)
        if key and key not in out:
            out[key] = row
    return out


def _write_member_row(ws, row: int, rec: Any) -> None:
    ws.cell(row, 1).value = getattr(rec, "name", "") or None
    ws.cell(row, 2).value = ws.cell(row, 2).value or ("A" if getattr(rec, "pid", "") else "D")
    ws.cell(row, 5).value = getattr(rec, "pid", "") or None
    ws.cell(row, 6).value = getattr(rec, "sex", "") or None
    ws.cell(row, 7).value = _date_value(getattr(rec, "bday", None))
    ws.cell(row, 11).value = _date_value(getattr(rec, "last_visit", None))
    ws.cell(row, 12).value = _num_or_blank(getattr(rec, "count_114", 0))
    ws.cell(row, 13).value = _num_or_blank(getattr(rec, "amount_114", 0))
    ws.cell(row, 14).value = _num_or_blank(getattr(rec, "count_115", 0))
    ws.cell(row, 15).value = _num_or_blank(getattr(rec, "amount_115", 0))


def _write_doctor_row(ws, row: int, rec: Any, months_115: int) -> None:
    ws.cell(row, 1).value = getattr(rec, "pid", "") or None
    ws.cell(row, 2).value = getattr(rec, "name", "") or None
    ws.cell(row, 3).value = _date_value(getattr(rec, "bday", None))
    ws.cell(row, 10).value = _date_value(getattr(rec, "last_visit", None))
    ws.cell(row, 11).value = _num_or_blank(getattr(rec, "count_114", 0))
    ws.cell(row, 12).value = _num_or_blank(getattr(rec, "count_114_q1", 0))
    ws.cell(row, 13).value = _num_or_blank(getattr(rec, "count_115", 0))
    amount_114 = GENERIC.parse_float(getattr(rec, "amount_114", 0)) or 0.0
    amount_115 = GENERIC.parse_float(getattr(rec, "amount_115", 0)) or 0.0
    ws.cell(row, 14).value = int(round(amount_114 / 12.0)) if amount_114 else None
    ws.cell(row, 15).value = int(round(amount_115 / float(max(months_115, 1)))) if amount_115 else None
...
def _postprocess_output(output_path: Path, source_dir: Path) -> None:
    ...
    records = [
        rec
        for rec in reader.collect_data(source_dir).values()
        if _record_has_month_data(rec) and (getattr(rec, "pid", "") or _rec_key(rec))
    ]
    if not records:
        return

    months_115 = _effective_115_months(source_dir, records)
    ...
    member_id_rows = _id_rows(ws_member, 5)
    doctor_id_rows = _id_rows(ws_doctor, 1)
    member_key_rows = _key_rows(ws_member, 1, 7)
    doctor_key_rows = _key_rows(ws_doctor, 2, 3)
    member_template_row = ws_member.max_row
    doctor_template_row = ws_doctor.max_row

    for rec in records:
        pid = GENERIC.normalize_id(getattr(rec, "pid", ""))
        key = _rec_key(rec)

        member_row = member_id_rows.get(pid) if pid else None
        if member_row is None and key:
            member_row = member_key_rows.get(key)
        if member_row is None:
            member_row = ws_member.max_row + 1
            _copy_row_style(ws_member, member_template_row, member_row)
            if pid:
                member_id_rows[pid] = member_row
            if key:
                member_key_rows[key] = member_row
        _write_member_row(ws_member, member_row, rec)

        doctor_row = doctor_id_rows.get(pid) if pid else None
        if doctor_row is None and key:
            doctor_row = doctor_key_rows.get(key)
        if doctor_row is None:
            doctor_row = ws_doctor.max_row + 1
            _copy_row_style(ws_doctor, doctor_template_row, doctor_row)
            if pid:
                doctor_id_rows[pid] = doctor_row
            if key:
                doctor_key_rows[key] = doctor_row
        _write_doctor_row(ws_doctor, doctor_row, rec, months_115)
