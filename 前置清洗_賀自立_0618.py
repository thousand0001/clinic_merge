...
指定欄位：
- G 看診數       → 會員總表 N（115 年看診次數；來源沒有 114 年看診次數）
- H 最後看診日   → 會員總表 K
- I 去年費用     → 會員總表 M
- J 今年費用     → 會員總表 O（115 年費用；有效月份算到 H 欄最後看診日的最大月份）
- Q 成健執行日   → 會員總表 Q
- O 子抹執行日   → 會員總表 R
- P 潛血執行日   → 會員總表 T
...
def last_115_visit_month(rows: Sequence[Dict[str, Any]]) -> int:
    months = [
        row["last_visit"].month
        for row in rows
        if isinstance(row.get("last_visit"), dt.date)
        and row["last_visit"].year - 1911 == 115
    ]
    return max(months) if months else 1


def split_number_by_months(total: float, month_count: int) -> List[float]:
    ...
    if total.is_integer():
        total_int = int(total)
        quotient, remainder = divmod(total_int, month_count)
        return [
            float(quotient + (1 if index < remainder else 0))
            for index in range(month_count)
        ]
    values = [total / month_count for _ in range(month_count)]
...
def write_claims(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    last_month = last_115_visit_month(rows)
    sheets_115 = []
    for month in range(1, last_month + 1):
        ws115 = wb.create_sheet(f"115{month:02d}")
        ws115.append(headers_115)
        sheets_115.append(ws115)
    for row in rows:
        ws114.append([row["pid"], row["name"], 0, row["amount_114"]])
        count_parts = split_number_by_months(row["count"], last_month)
        amount_parts = split_number_by_months(row["amount_115"], last_month)
        for index, ws115 in enumerate(sheets_115):
            ws115.append([
                row["pid"],
                row["name"],
                row["last_visit"],
...
def write_clean_data(
    ...
    return {
        ...
        "count_n": sum(row["count"] for row in rows),
        "amount_o": sum(row["amount_115"] for row in rows),
        "last_115_month": last_month,
...
def prepare_clean_dir(
    ...
    counters["selection_order_rows"] = rows
...
def load_core_module():
    core_path = latest_core()
    spec = importlib.util.spec_from_file_location("hezili_common_core_patch", core_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"無法載入共用核心：{core_path}")
    core = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = core
...
def _append_category(value: Any, flag: str) -> str:
    parts = [part for part in normalize_text(value).split("/") if part]
    if flag not in parts:
        parts.append(flag)
    if "D" in parts and len(parts) > 1:
        parts = [part for part in parts if part != "D"]
    order = ["A", "B", "E1", "E2", "D"]
    parts.sort(key=lambda part: order.index(part) if part in order else len(order))
    return "/".join(parts) if parts else flag


def _copy_self_select_row_style(ws: Any, target_row: int, styles: List[Dict[str, Any]]) -> None:
    for index, style in enumerate(styles, start=1):
        cell = ws.cell(target_row, index)
...
def patch_self_select_order(output_path: Path, source_rows: Sequence[Dict[str, Any]]) -> int:
    core = load_core_module()
    wb = openpyxl.load_workbook(output_path)
    try:
        if core.Rules.SHEET_TARGET not in wb.sheetnames or core.SELF_SELECT_SHEET_NAME not in wb.sheetnames:
            return 0
        ws_main = wb[core.Rules.SHEET_TARGET]
        ws_self = wb[core.SELF_SELECT_SHEET_NAME]
        cols = core.detect_template_columns(ws_main, core.Rules.DATA_START_ROW)

        main_by_id: Dict[str, int] = {}
        main_by_name: Dict[str, List[int]] = defaultdict(list)
        for row_no in range(core.Rules.DATA_START_ROW, ws_main.max_row + 1):
            pid = normalize_id(ws_main.cell(row_no, cols["id"]).value if cols.get("id") else "")
            name = normalize_name(ws_main.cell(row_no, cols["name"]).value if cols.get("name") else "")
            if pid:
                main_by_id.setdefault(pid, row_no)
            if name:
                main_by_name[name].append(row_no)

        max_col = max(ws_self.max_column, 26)
        existing_by_id: Dict[str, List[Any]] = {}
        existing_by_name: Dict[str, List[Any]] = {}
        for row_no in range(3, ws_self.max_row + 1):
            values = [ws_self.cell(row_no, col).value for col in range(1, max_col + 1)]
            name = normalize_name(values[0] if values else "")
            pid = normalize_id(values[1] if len(values) > 1 else "")
            if pid:
                existing_by_id.setdefault(pid, values)
            if name:
                existing_by_name.setdefault(name, values)

        template_styles: List[Dict[str, Any]] = []
        template_row = 3 if ws_self.max_row >= 3 else 2
        for col in range(1, max_col + 1):
            cell = ws_self.cell(template_row, col)
        ...
        if ws_self.max_row >= 3:
            ws_self.delete_rows(3, ws_self.max_row - 2)

        src_col_map = core._build_src_col_map(core._SELF_SELECT_COL_MAP, cols)
        self_col_indexes = {
            key: openpyxl.utils.column_index_from_string(letter)
            for key, letter in core._SELF_SELECT_COL_MAP
        }
        selected_count = 0
        seen_keys: set[Tuple[str, str]] = set()
        for source in source_rows:
            if source.get("source_kind") != "select":
                continue
            source_pid = normalize_id(source.get("pid"))
            source_name = normalize_name(source.get("name"))
            key = (source_pid, source_name)
            if key in seen_keys:
                continue
            seen_keys.add(key)

            main_row = main_by_id.get(source_pid)
            if main_row is None:
                candidates = main_by_name.get(source_name, [])
                if len(candidates) == 1:
                    main_row = candidates[0]

            values = existing_by_id.get(source_pid) or existing_by_name.get(source_name)
            if values is None:
                values = [None] * max_col
                values[self_col_indexes["name"] - 1] = source.get("name")
                # ID 格式異常者在正式總表仍留空；自選分頁同樣留空，避免誤作有效 ID。
                values[self_col_indexes["id"] - 1] = source_pid if is_valid_id(source_pid) else None
                if main_row is not None:
                    for key_name, letter in core._SELF_SELECT_COL_MAP:
                        src_col = src_col_map.get(key_name)
                        if src_col:
                            values[openpyxl.utils.column_index_from_string(letter) - 1] = ws_main.cell(main_row, src_col).value

            if main_row is not None:
                if cols.get("is_self_select"):
                    ws_main.cell(main_row, cols["is_self_select"]).value = "✔"
                if cols.get("abc"):
                    ws_main.cell(main_row, cols["abc"]).value = _append_category(ws_main.cell(main_row, cols["abc"]).value, "E1")

            out_row = ws_self.max_row + 1
            _copy_self_select_row_style(ws_self, out_row, template_styles)
            for col, value in enumerate(values, start=1):
                ws_self.cell(out_row, col).value = value
            selected_count += 1
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="clinic_merge_hezili_") as tmp:
        ...
        counters["self_output"] = patch_self_select_order(output, counters["selection_order_rows"])
    print(
        f"完成：{output}\n"
